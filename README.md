...
3. **Two independent, fresh-context adversarial verifiers** grade the *artifacts* (diff, tests, migration logs) — one on contract/correctness, one hunting silent failures, invented conventions, and scope creep. Both must pass.
...
```
docs/
  specs/       Canonical Phase 0–6 specifications (the build target of record)
  decisions/   Architecture Decision Records (9 ADRs)
  planning/    Verified spec review + the autonomous-loop build plan
```
*(Application code — `apps/`, `packages/`, `fos-evals/` — lands with the Phase-0 scaffold PR.)*
...
*License: not yet set (all rights reserved by default). Built in the open as an engineering showcase.*
